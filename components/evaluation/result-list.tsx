  ...
  if (!result.metrics.length) {
    return (
      <p className="results-empty">
        No normalized evaluation metrics recorded.
      </p>
    );
  }
        ...
        return (
          <details
            className={`result-entry ${
              unexpectedFailure ? "result-entry-attention" : ""
            }`}
            key={`${result.id}-${index}`}
            open={unexpectedFailure}
          >
            <summary>
              ...
              <span className="result-time">
                {formatSeconds(result.responseTimeSeconds)}
              </span>
            ...
            <div className="result-detail">
              <div className="result-narrative">
                <section>
                  <span className="result-label">Prompt</span>
                  <p>{result.prompt}</p>
                </section>

                <section>
                  <span className="result-label">Actual response</span>
                  <pre>{result.actualResponse}</pre>
                </section>

                <section>
                  <span className="result-label">Evaluation reason</span>
                  <p>{result.reason}</p>
                </section>
              </div>
              ...
              <aside className="result-evidence">
                ...
                <div className="result-fact">
                  <span>Assertion</span>
                  <strong>{result.assertionType}</strong>
                </div>
                ...
                <div className="result-fact-grid">
                  <div>
                    ...
                    <strong>{formatNumber(result.promptTokens)}</strong>
                  </div>

                  <div>
                    <span>Output tokens</span>
                    <strong>{formatNumber(result.outputTokens)}</strong>
                  </div>

                  <div>
                    <span>Prompt latency</span>
                    <strong>
                      {formatSeconds(result.promptLatencySeconds)}
                    </strong>
                  </div>
                  ...
                  <div>
                    ...
                    <strong>
                      {formatSeconds(result.generationLatencySeconds)}
                    </strong>
                  </div>
                  ...
                  <div>
                    ...
                    <strong>{formatUsd(result.estimatedCostUsd)}</strong>
                  </div>
                </div>
              </aside>
